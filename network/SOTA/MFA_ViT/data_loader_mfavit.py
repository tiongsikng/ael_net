...
from network.SOTA.MFA_ViT.params_mfavit import batch_sub, batch_samp, seed, device, random_batch_size, test_batch_size
...
def gen_data(path_dir, mode, type='periocular', aug='False'):
    ...
    aug_trans = transforms.Compose([
                ...
                transforms.RandomResizedCrop(size=sz, scale=(0.8, 1.0),
                                             ratio=(0.75, 1.33)),
    ...
    if mode == 'train':
        data_loader = torch.utils.data.DataLoader(data_sets, batch_sampler = data_sampler, num_workers = 2,
                                              worker_init_fn = random.seed(seed))
    elif mode == 'train_rand':
        data_loader = torch.utils.data.DataLoader(data_sets, batch_size = random_batch_size, num_workers = 2,

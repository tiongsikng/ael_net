...
from network.SOTA.MFA_ViT.regularizer import DropPath
...
class MFA_ViT(nn.Module):
    ...
    def __init__(self, attr_size=47, img_size=112, patch_size=8, in_chans=3, embed_dim=1024, num_classes=9131,
        ...
        self.peri_tokenizer = PatchEmbed(img_size=img_size, view=1, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
    ...
    def forward(self, x, peri_flag=False):
        """
            ...
            :return: Face and periocular classification heads
        """
        mode = "ocular" if peri_flag == True else "face"

        x = self.tokenize(x, mode=mode)
        x = self.forward_features(x, mode=mode)
        y = self.head(x)

        return F.normalize(x, dim=1, p=2), y
...
if __name__ == '__main__':
    ...
    model = MFA_ViT(attr_size=47, img_size=112, patch_size=8, in_chans=3, embed_dim=1024, num_classes=9131,

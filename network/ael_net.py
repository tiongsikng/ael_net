...
class AEL_Block(nn.Module):
    def __init__(self, channels, num_heads, expansion_factor):
        super(AEL_Block, self).__init__()

        self.norm1 = nn.LayerNorm(channels)
        self.attn = CMSA(channels, num_heads)
...
class AEL_Net(Module):
    ...
    def forward(self, x, peri_flag = False):
        ...
        out = self.encoder_64_1(out)
        out = self.conv_23(out)
        out = self.conv_3(out)
        out = self.encoder_64_2(out)
        out = self.conv_34(out)
        out = self.conv_4(out)
        out = self.encoder_ori(out)
...
if __name__ == '__main__':
    ...
    model = AEL_Net(embedding_size = 1024, do_prob=0.0).eval().to(device)
